...
class ServerConnection:
    ...
    def load_message_flags(self):
        ...
        flags_str = FileManager.read_json(
            join(dirname(abspath(__file__)), "server_message_flags.json")
        )
        return flags_str
    ...
    def start_server(self):
        try:
            ...
            print(
                f"[ServerConnection.start_server] Error binding or starting the server: {e}"
            )
            self.server_running = False
    ...
    def accept_connections(self):
        while self.server_running and self.current_connections < self.max_connections:
            try:
                ...
                print(
                    f"[ServerConnection.accept_connections] Socket timeout occurred: {e}"
                )

            except socket.error as e:
                print(
                    f"[ServerConnection.accept_connections] Socket error during connection acceptance: {e}"
                )
        ...
        try:
            if self.current_connections < self.max_connections:
                ...
                try:
                    ...
                    print(
                        f"[ServerConnection.handle_client] Error sending connection accepted message: {e}"
                    )
            else:
                print(
                    f"Connection with {self.client_address} rejected - connections limit reached!"
                )
                try:
                    self.client_socket.sendall(
                        b"Connection rejected by server!" + self.message_flags["INFO"]
                    ...
                    print(
                        f"[ServerConnection.handle_client] Error sending rejection message: {e}"
                    )
                finally:
                    self.client_socket.close()
    ...
    def send_file_request(self, file_name: str, file_size: str, chunk_size: int):
        ...
        try:
            ...
            print(
                f"[ServerConnection.send_file_request] Error sending file metadata: {e}"
            )
        except Exception as e:
    ...
    def send_EOF(self):
        try:
            self.client_socket.sendall(
                self.message_flags["END"] + self.message_flags["INFO"]
            )
        except socket.error as e:
    ...
    def receive_answer(self):
        ...
        try:
            ...
            print(
                f"[ServerConnection.receive_answer] Error while receiving response: {e}"
            )
        except Exception as e:
        ...
        try:
            ...
            for i in range(0, len(file_data), chunk_size):
                ...
                try:
                    ...
                    print(
                        f"[ServerConnection.send_file] Socket error while sending the file: {e}"
                    )
                except Exception as e:
                    print(
                        f"[ServerConnection.send_file] An error occurred while sending the file: {e}"
                    )
            print("Done...")
        except Exception as e:
    ...
    def send_message(self, message):
        ...
        try:
            ...
            self.client_socket.sendall(
                message_json.encode("utf-8") + self.message_flags["INFO"]
            )
    ...
    def stop_server(self):
        try:
            ...
            print(
                f"[ServerConnection.stop_server] Error closing the server socket: {e}"
            )
        except Exception as e:
